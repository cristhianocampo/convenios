...
<head> 
	<meta charset="UTF-8">
	<title>Institución | Mostrar</title>
...
<body>
	...
	<div class="container-fluid">
		<section class="row gestion">
			<div class="container">
				<h3 class="titulo">Listado de Perfil de la Institución</h3>
			</div>
		...
		<section class="row main">
			...
			<div class="row martop">
					...
					<br>
						<form class="form-inline" role="search" method="get" action="{{url('/institucion/busqueda')}}">
							{{ csrf_field() }}
				...
				<br>
					<div class="table-responsive">
						<table class="table table-bordered table-hover table-condensed">
							...
							<tr>
								<td><a href="{{asset("institucion/$ins[idinstitucion]/editar")}}">{{$ins->nombreinstitucion}}</a></td>				
								<td>{{$ins->tipo}}</td>
								<td>{{$ins->existencia}}</td>
								<td>{{$ins->nestudiantes}}</td>
								<td>{{$ins->ndocentes}}</td>
								<td>{{$ins->carreras}}</td>
							</tr>
							@endforeach
