...
<head> 
	<meta charset="UTF-8">
	<title>Convenio | Mostrar</title>
	<meta name="viewport" content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
	<link rel="stylesheet" href="{{ url('css/stylistuser.css') }}">
...
<body>
	...
	<div class="container-fluid">
		<section class="row gestion">
			<div class="container">
				<h3 class="titulo">Listado Convenios</h3>
			</div>
		...
		<section class="row main">
			...
			<div class="row martop">
					...
					<br>
						<form class="form-inline" role="search" action="{{url('/convenio/busqueda')}}">
        					<div class="form-group">        						
        						<select name="info" id="" class="form-control" required>
        							<option value="" disabled selected>Seleccione</option>
          							<option value="nombre">Nombre</option>
          							<option value="tipo">Tipo</option>
          							<option value="procedencia">Procedencia</option>
          							<option value="institucion">Institucion</option>
          							<option value="frenovacion">Forma de renovación</option>
          							<option value="pais">País</option>
          							<option value="estado">Estado</option>
          							<option value="pe">Persona de enlace</option>
          						</select>
							...
							<li class="dropdown" style="display: inline-block; float: right;">
							   	<a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"><span style="color:black;">Búsqueda avanzada | Seleccione</span><span class="caret" style="color: black;"></span></a>
							   	<ul class="dropdown-menu">
							        <li><a href="{{url('/convenio/reporte/institucion-vigencia')}}">Institución y período de vigencia</a></li>
							        <li><a href="{{url('/convenio/reporte/institucion-estado')}}">Institución y Estado</a></li><li><a href="{{url('/convenio/reporte/país-firma')}}">País y Fecha de firma</a></li>       							       
							        <li><a href="{{url('/convenio/reporte/formaderenovacion-procedencia')}}">Forma de renovación y Procedencia</a></li>       				
							   	</ul>
							</li>
					...
					<div class="table-responsive">
						<table class="table table-bordered table-hover table-condensed">
							<tr class="active">
								...
								<th>Procedencia</th>
								<th>Institución</th>
								<th>Forma de renovación</th>
								<th>País</th>
								<th>Período de vigencia</th>
								<th>Persona de enlace</th>
								<th>Estado</th>
								<th>Acción</th>
							</tr>
							@foreach($convenio as $con)
							<tr>
								<td><a href="{{asset("/convenio/$con[idconvenio]/perfil")}}">{{$con['nombreconve']}}</a></td>		
								<td>{{$con->tipoconve}}</td>
								<td>{{$con->fechafirma}}</td>
								<td>{{$con->procedencia}}</td>
								<td>{{$con->nombreinstitucion}}</td>
								<td>{{$con->frenovacion}}</td>
								<td>{{$con->pais}}</td>
								<td>De {{$con->fi}} a {{$con->ff}}</td>
								<td>{{$con->nombreenlace}}</td>
								<td>{{$con->nombreestado}}</td>	
								<td><a href="{{asset("convenio/$con[idconvenio]/editar")}}">Editar</a></td>
							</tr>
							@endforeach
						</table>
						{!! $convenio->render() !!}
